...
def aug_random_edge(input_adj, drop_percent=0.2):
    ...
    """
    # 注意：这里先除以 2，再在后面 /2 一次，因此实际删/加边数量会比 drop_percent 更小。
    percent = drop_percent / 2
    ...
    # 由于无向图存在 (i,j)/(j,i) 两个方向，下面去重为“单向列表” single_index_list
    processed_edges = set()
    single_index_list = []
    for i in index_list:
    ...
    # 2) 随机添加边（从所有可能的 (i,j) 对中采样，j<i 保证不重复）
