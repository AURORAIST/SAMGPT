...
def build_subgraph(adj, idx_train, sparse=True, subgraph_hop=2, max_neighbors=None):
    ...
    for x in range(idx_train.shape[0]):
        ...
        if subgraph_hop <= 1:
            cur_nodes = [idx_train_list[x]] + neighborslist[x]
        else:
            cur_nodes = [idx_train_list[x]] + neighborslist[x] + neighbors_2hoplist[x]

        if max_neighbors is not None and len(cur_nodes) > max_neighbors:
            center = [idx_train_list[x]]
            one_hop = neighborslist[x]
            two_hop = neighbors_2hoplist[x]
            remaining = max_neighbors - 1
            keep_one = one_hop[:max(0, remaining)]
            remaining -= len(keep_one)
            keep_two = two_hop[:max(0, remaining)] if remaining > 0 else []
            cur_nodes = center + keep_one + keep_two

        # 子图节点集合：中心节点 + 1-hop + 2-hop
        mainlist[x] = cur_nodes
