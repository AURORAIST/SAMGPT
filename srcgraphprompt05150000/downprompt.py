...
class downprompt(nn.Module):
    ...
    def __init__(
        ...
        self.readout_query = nn.Parameter(torch.randn(ft_in))

    def _emit_eval_log(self, message):
        print(message)
        logging.info(message)

    def forward(self, features, adj, sparse, gcn, idx, labels=None, train=0, args=None, adj_for_subgraph=None, adj_for_subgraph_is_sparse=True):
        # 生成全图 embedding，然后取出 idx 对应的样本
        embeds = self.downstreamPrompt(features, gcn, adj, sparse).squeeze(0)

        use_subgraph = (
            args is not None
            and getattr(args, 'downstream_task', 'node') == 'node'
            and getattr(args, 'query_mode', 'node') == 'subgraph'
        )

        if not use_subgraph:
            self._emit_eval_log('[EVAL] downstream_task=node')
            self._emit_eval_log('[EVAL] query_mode=node')
            self._emit_eval_log('[EVAL] representation_source=node_embedding')
            rawret = embeds[idx]
            num = rawret.shape[0]
            if train == 1:
                self.ave = averageemb(labels=labels, rawret=rawret)
            rawret = torch.cat((rawret, self.ave.to(rawret.device)), dim=0)
            rawret = torch.cosine_similarity(rawret.unsqueeze(1), rawret.unsqueeze(0), dim=-1)
            ret = rawret[:num, num:]
            ret = F.softmax(ret, dim=1)
            return ret

        if adj_for_subgraph is None:
            self._emit_eval_log('[EVAL] downstream_task=node')
            self._emit_eval_log('[EVAL] query_mode=subgraph')
            self._emit_eval_log('[EVAL] representation_source=node_embedding_fallback')
            rawret = embeds[idx]
            num = rawret.shape[0]
            if train == 1:
                self.ave = averageemb(labels=labels, rawret=rawret)
        ...
        subgraph = process.build_subgraph(
            adj_for_subgraph,
            idx.detach().cpu() if isinstance(idx, torch.Tensor) else idx,
            sparse=adj_for_subgraph_is_sparse,
            subgraph_hop=getattr(args, 'subgraph_hop', 2),
            max_neighbors=getattr(args, 'max_neighbors', None),
        )

        sub_idx = subgraph['idx'].to(embeds.device)
        sub_batch = subgraph['batch'].to(embeds.device)
        sub_embeds = embeds[sub_idx]

        scores = torch.matmul(sub_embeds, self.readout_query.to(sub_embeds.device).unsqueeze(1)).squeeze(1)
        group_max = torch_scatter.scatter(scores, sub_batch, dim=0, reduce='max')
        normalized = torch.exp(scores - group_max[sub_batch])
        group_sum = torch_scatter.scatter(normalized, sub_batch, dim=0, reduce='sum')
        weights = normalized / (group_sum[sub_batch] + 1e-12)
        readout = torch_scatter.scatter(sub_embeds * weights.unsqueeze(1), sub_batch, dim=0, reduce='sum')

        sizes = torch_scatter.scatter(torch.ones_like(sub_batch, dtype=torch.float), sub_batch, dim=0, reduce='sum')
        avg_size = float(sizes.float().mean().item()) if sizes.numel() else 0.0
        max_size = int(sizes.max().item()) if sizes.numel() else 0

        if train == 1:
            self.ave = averageemb(labels=labels, rawret=readout)

        if not hasattr(self, '_printed_eval_logs'):
            self._printed_eval_logs = {'train': False, 'test': False}
        phase = 'train' if train == 1 else 'test'
        if not self._printed_eval_logs[phase]:
            self._emit_eval_log('[EVAL] downstream_task=node')
            self._emit_eval_log('[EVAL] query_mode=subgraph')
            self._emit_eval_log('[EVAL] representation_source=subgraph_readout')
            self._emit_eval_log(f"[EVAL] max_neighbors_per_node={getattr(args, 'max_neighbors', None)}")
            self._emit_eval_log(f'[EVAL] avg_support_subgraph_size={avg_size}' if train == 1 else '[EVAL] avg_support_subgraph_size=N/A')
            self._emit_eval_log(f'[EVAL] avg_query_subgraph_size={avg_size}' if train == 0 else '[EVAL] avg_query_subgraph_size=N/A')
            self._emit_eval_log(f'[EVAL] max_subgraph_size_effective={max_size}')
            self._emit_eval_log(f'[EVAL] support_repr_shape={tuple(readout.shape)}' if train == 1 else '[EVAL] support_repr_shape=N/A')
            self._emit_eval_log(f'[EVAL] query_repr_shape={tuple(readout.shape)}' if train == 0 else '[EVAL] query_repr_shape=N/A')
            self._printed_eval_logs[phase] = True

        num = readout.shape[0]
        rawret = torch.cat((readout, self.ave.to(readout.device)), dim=0)
        rawret = torch.cosine_similarity(rawret.unsqueeze(1), rawret.unsqueeze(0), dim=-1)
        ret = rawret[:num, num:]
        ret = F.softmax(ret, dim=1)
        return ret
